...
# find close
        ...
        if num_ants>=200:
            time_to_spare = (ants.turntime/1000.0) - (-0.003512 + 0.00047632*num_ants -0.00000105286*(num_ants*num_ants)) - 0.03 
        elif num_ants<=60:
            time_to_spare = (ants.turntime/1000.0) - 0.03
        else:
            time_to_spare = (ants.turntime/1000.0) - (-0.003512 + 0.00047632*num_ants -0.00000105286*(num_ants*num_ants)) - 0.01 
	...
	#MY HILLLLSSS                
        ...
        for i in range(ants.rows):
            for j in range(ants.cols):
                if self.grid[i][j]!=self.MYHILL and self.grid[i][j]!=self.ENEMYHILL and self.grid[i][j]!=self.MYHILL2:
...
#                        sys.stderr.flush()
        ...
        for ant_loc in ants.my_ants():
            ...
            if self.grid[antrow][antcol]!=self.MYHILL and self.grid[antrow][antcol]!=self.MYHILL2:
        ...
        for enemy_loc, enemy_owner in ants.enemy_ants():
            ...
            if self.grid[enemyrow][enemycol]!=self.ENEMYHILL:
                self.grid[enemyrow][enemycol]=self.ENEMYANTS
            
            #if they're near my base, retreat to base
            for hill_loc in ants.my_hills():
                x, y = hill_loc
                if ants.distance(hill_loc, enemy_loc)<ants.attackradius2+1:
#                    print >> sys.stderr, 'ant retreat', enemyrow, enemycol
#                    sys.stderr.flush()
            ...
            for ant_loc in ants.my_ants():
                antrow, antcol=ant_loc
                if ants.distance(ant_loc, enemy_loc)<ants.attackradius2+1:
                    friends=friends+1

            enemies=0
##            for enemy_loc2, enemy_owner2 in ants.enemy_ants():
##                if ants.distance(enemy_loc, enemy_loc2)<ants.attackradius2+3:
##                    enemies=enemies+1
                    
            if friends>=2:
                if self.grid[enemyrow][enemycol]!=self.ENEMYHILL:
                    self.grid[enemyrow][enemycol]=self.ENEMYANTS2
#                    print >> sys.stderr, 'u surrounded em, attack!!!!!!!'
#                    sys.stderr.flush()


	#BOUNDARY EXPANDING !!!!!!!!! I KNOW IT AINT RIGHT :P
        for i in range(ants.rows):
            for j in range(ants.cols):
